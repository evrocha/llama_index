...
def process_pdf(file):
   with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
        ...
        with open(temp_file.name, encoding='latin1') as pdf_file:
...
def main(query):
  ...
  loader = PDFReader()
  documents = loader.load_data(file=Path(process_pdf(file=uploaded_file)))
  QA_PROMPT_TMPL = (
...
# import json
# path = './data/DONE_NL103_shannon.pdf'
# print(open(path, encoding="latin1").readline())
# records = [json.dumps(line) for line in open(path, encoding="latin1")]
